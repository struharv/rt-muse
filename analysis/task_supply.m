function experim_json = task_supply(experiment_name,experim_json,task_id)
  ...
  slbf_file = [tasks_names{task_id}, '/supply.slbf.csv'];
  ...
  subf_file = [tasks_names{task_id}, '/supply.subf.csv'];
  ...
  % upper bound defined as: min(seq_slope*t, alpha*t+burst)
  % with t spanning over [0,horizon], seq_slope set to 1 for single task
