...
%   function is in the current directory.
	...
	% The number of jobs used to compute the minimum job lengths is the
	% minimum between idx_last_ref and portion_ref percent of the
	% available jobs.
	idx_last_ref = 1000;
	portion_ref = .01;
	...
	% the following tolerance is used. If set to zero, then the exact (with
	% many point) supply function is considered. A value of 1e-5 was shown to
	...
	% approximation. It should be significantly smaller than the simulation
	% time specified in the .json file ("global.duration"), like one order of
	% magnitude less.
	time_horizon = 1;
	...
	% average of the minimum separation of reference sequence
	ind_last_ref = min(ceil(num_ref_marks*portion_ref), idx_last_ref);
	lowb_all_y = linspace(0,ref_data(ind_last_ref,1)*(num_sim_marks-1)/(ind_last_ref-1),
		num_sim_marks)';

	lowb_all_x = sim_data(:,3); % max separations

	% computing the inner corners of the supply function
	...
	% linear lower bound over [0, time_horizon]
